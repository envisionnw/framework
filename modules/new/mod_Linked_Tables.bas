...
'               	BLC, 6/12/2015 - 1.03 - replaced mod_Linked_TablesVars.item(... with mod_Linked_TablesVars("...
...
'               BLC, 7/31/2014 - changed gvars to mod_Linked_TablesVars, shifted to initApp module
...
'               BLC, 6/12/2015 - replaced mod_Linked_TablesVars.item("... with mod_Linked_TablesVars("...
...
Public Function VerifyConnections()
    ...
    Set db = CurrentDb
    mod_Linked_TablesVars.Item("Connected") = False           ' Default in case of error
    mod_Linked_TablesVars.Item("HasAccessBE") = False         ' Flag to indicate that at least 1 Access BE exists
    ...
    Do Until rs.EOF
        ...
        Else
            ...
            mod_Linked_TablesVars.Item("HasAccessBE") = True
    ...
    If DB_ADMIN_CONTROL = True Then
        ...
        If FormIsOpen("frmSwitchboard") And blnHasError = False Then
            If Forms!frm_Switchboard.fsub_DbAdmin.Form.chkVerifyOnStartup Then
                If mod_Linked_TablesVars.Item("HasAccessBE") = True Then
...
Proc_Final_Status:
    ...
    Else  ' If no connection errors, then set the global variable flag to True
        mod_Linked_TablesVars.Item("Connected") = True
    End If
...
Public Function LinkedDatabase(ByVal strTableName As String) As String
    ...
    Dim strmod_Linked_Tables As String

    strmod_Linked_Tables = ParseConnectionStr(CurrentDb.TableDefs(strTableName).connect)
    LinkedDatabase = strmod_Linked_Tables
...
Public Function RefreshLinks(strDbName As String, ByVal strNewConnStr As String, _
    ...
    Set db = CurrentDb
    Set rs = db.OpenRecordset(Getmod_Linked_Tableslate("s_tsys_link_tables_by_dbname", "dbName" & PARAM_SEPARATOR & strDbName), dbOpenSnapshot)
...
Debug.Print strDesc
			...
			strSQL = Getmod_Linked_Tableslate("u_tsys_link_tables_description", "descr" & PARAM_SEPARATOR & strDesc & "|tbl" & PARAM_SEPARATOR & strTable)
'
...
'                rs![Link_table] & """);"
            strSQL = Getmod_Linked_Tableslate("d_tsys_link_tables", "linktbl" & PARAM_SEPARATOR & rs![LinkTable])
            DoCmd.SetWarnings False
            DoCmd.RunSQL strSQL
            DoCmd.SetWarnings True
            rs.MoveNext
...
'            "FROM qsys_Linked_tables_not_in_tsys_Link_Tables;"
        strSQL = Getmod_Linked_Tableslate("i_tsys_link_tables")
        DoCmd.RunSQL strSQL
        DoCmd.SetWarnings True
        ' Update descriptions
'        Set rs = db.OpenRecordset("SELECT * FROM tsys_Link_Tables " & _
'            "WHERE tsys_Link_Tables.Description_text Is Null", dbOpenSnapshot)
        Set rs = db.OpenRecordset(Getmod_Linked_Tableslate("s_tsys_link_tables_no_description"), dbOpenSnapshot)
        Do Until rs.EOF
            strTable = rs![LinkTable]
            Set tdf = db.TableDefs(strTable)
...
'                rs![Link_db] & """);"
            strSQL = Getmod_Linked_Tableslate("d_tsys_link_tables_by_db", "link_db" & PARAM_SEPARATOR & rs![LinkDb])
            DoCmd.SetWarnings False
            DoCmd.RunSQL strSQL
            DoCmd.SetWarnings True
            rs.MoveNext
...
'        "ORDER BY MSysObjects.Name;", dbOpenSnapshot)
    Set rs = CurrentDb.OpenRecordset(Getmod_Linked_Tableslate("s_msysobjects_except_deleted"), dbOpenSnapshot)
...
Public Sub FixLinkedDatabase(ByVal strTableName As String)
    ...
    Dim strmod_Linked_Tables As String, strSQL As String, strCurDb As String, strCurDbPath As String
    Dim rs As DAO.Recordset
...
'                     "WHERE LinkDb = '" & strCurDb & "';"
            strSQL = Getmod_Linked_Tableslate("", "curDbPath" & PARAM_SEPARATOR & strCurDbPath & "|curDb" & PARAM_SEPARATOR & strCurDb)
