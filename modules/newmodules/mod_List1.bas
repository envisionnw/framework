...
'               BLC, 6/12/2015 - 1.01 - updated documentation, mod_List1Vars("... vs. mod_List1Vars.item("...
...
On Error GoTo Err_Handler
    ...
    'save headers
    mod_List1Vars.Add "lbxHdr", strColHeads
...
'   BLC - 5/10/2015 - moved to mod_List from mod_Lists
'   BLC - 6/12/2015 - replaced mod_List1Vars.item("... with mod_List1Vars("...
' ---------------------------------
Public Sub SortList(lbx As ListBox) ', orderCol As Integer)

On Error GoTo Err_Handler
  
  Dim strmod_List1 As String
  Dim i As Integer, iHdr As Integer
  Dim j As Integer
  
  'skip first row if lbx has headers
  iHdr = 0
  If Len(mod_List1Vars("lbxHdr")) > 0 Then
    iHdr = 1
  End If
  ...
  For i = iHdr To lbx.ListCount - 1
    For j = i + 1 To lbx.ListCount - 1
      If lbx.ItemData(i) > lbx.ItemData(j) Then
        strmod_List1 = lbx.ItemData(i)
        lbx.RemoveItem (i)
        lbx.AddItem lbx.ItemData(j - 1), i
        lbx.RemoveItem (j)
        lbx.AddItem strmod_List1, j - 1
       End If
     Next j
   Next i
...
'               This creates a mod_List1orary table for creating the recordset via DAO.
' Assumptions:  -
...
'               tblName - mod_List1orary table name (string)
'               blnReplace - true = replace records in the mod_List1 table (if it exists)
'                            false = append to records in the mod_List1 table (if it exists)
' Returns:      -
' Throws:       none
' References:   none
' Source/date:
...
'   BLC - 5/26/2015 - revised to SetListRecordset saving listbox rows to mod_List1 table
'   BLC - 5/27/2015 - added blnReplace to handle adding additional records to the mod_List1
'                     table from a list
' ---------------------------------
...
'            GoTo Exit_Handler
    ...
    If lbx.ListCount > 0 Then
        ...
        If Not blnTableExists Then
            ...
            'create mod_List1orary table (if it doesn't exist)
...
' Parameters:   tblName - mod_List1orary table name (string)
...
' Description:  Create a recordset from list items saved in mod_List1 table
...
On Error GoTo Err_Handler
    ...
    ' -------------------------------------------------------------------------
    If frm.Controls(strSourceControl).ColumnHeads = True Then
        frm.Controls(strSourceControl).AddItem mod_List1Vars("lbxHdr")
    End If
...
On Error GoTo Err_Handler
    ...
    If frm.Controls(strTargetControl).ColumnHeads = True And frm.Controls(strTargetControl).ListCount = 0 Then
       stritem = mod_List1Vars("lbxHdr") & stritem
       frm.Controls(strTargetControl).AddItem stritem
    End If
