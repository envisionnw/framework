Attribute VB_Name = "mod_Initialize_App"
...
'               BLC, 10/5/2016 - 1.05 - set AppVersion mod_Initialize_AppVar in AppSetup()
'				-----------------------------------------------------------------------
...
'                BLC, 7/31/2014 - changed db & user gvars to mod_Initialize_AppVars & initialized values
'                BLC, 8/6/2014  - switched order of setting globals & constants before sub
'                                 to ensure these load upon module being called for initGlobalmod_Initialize_AppVars
...
' ---------------------------------
' SUB:          initGlobalmod_Initialize_AppVars
' Description:  Initializes database mod_Initialize_AppVars which cannot be initialized outside of sub/function
' Assumptions:  -
' Parameters:   -
' Returns:      -
' Throws:       -
...
' Revisions:    BLC, 7/31/2014 - initial version
' ---------------------------------
Public Sub initGlobalmod_Initialize_AppVars()
On Error GoTo Err_Handler:
Dim aryStdVars() As Variant
Dim i As Integer

    ' Global variables
    mod_Initialize_AppVars.Add "Connected", False     'Boolean flag -> back-end db connection is valid or not
    mod_Initialize_AppVars.Add "HasAccessBE", False   'Boolean flag -> app has one or more Access back-ends or not
    
    ' User access global variables
    mod_Initialize_AppVars.Add "WritePermission", False   'Boolean flag -> user has write privileges to the back-end db or not
...
Err_Handler:
    Select Case Err.Number
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - initGlobalmod_Initialize_AppVars[mod_Initialize_App])"
    End Select
    Resume Exit_Procedure
End Sub
...
' Adapted:      -
' Revisions:    BLC, 7/31/2014 - initial version
'               BLC, 6/12/2015 - replaced mod_Initialize_AppVars.item("... with mod_Initialize_AppVars("...
' ---------------------------------
Public Sub initApp()
On Error GoTo Err_Handler:
    ...
    ' Initialize global mod_Initialize_AppVars that require function
    initGlobalmod_Initialize_AppVars
    ...
    VerifyConnections
    If mod_Initialize_AppVars.Item("Connected") Then AppSetup
...
'               BLC, 6/12/2014 - revised to set mod_Initialize_AppVars.Item("UserAccessLevel") vs. cAppMode
'                                mod_Initialize_AppVars available throughout app w/o setting cAppMode subform control
'               BLC, 7/31/2014 - changed gvars to mod_Initialize_AppVars, moved to mod_Initialize_App,
...
'               BLC, 10/5/2016 - set AppVersion mod_Initialize_AppVar
'				-----------------------------------------------------------------------
'				BLC, 8/22/2017 - 1.06 - merged in prior work:
'
'		               BLC, 6/12/2015 - replaced mod_Initialize_AppVars.item("... with mod_Initialize_AppVars("...
...
Public Function AppSetup()
    ...
    mod_Initialize_AppVars.Item("WritePermission") = False
...
'**********************************************
	...
	'strUser = UserName
    'strRelease = Left(strReleaseID, 8) & " / " & mod_Initialize_AppVars("UserAccessLevel")

    strRelease = Left(strRelease, InStr(strRelease, "(") - 2) & " / " & mod_Initialize_AppVars.Item("UserAccessLevel")
    ...
    'set app version
    mod_Initialize_AppVars.Add "AppVersion", strReleaseVersion
    strUser = Nz(mod_Initialize_AppVars.Item("AppUsername"), "PreLogin")
...
'            strRelease & "' AS Action_taken"
        strSQL = Getmod_Initialize_Applate("i_tsys_logins_odbc", "Username" & PARAM_SEPARATOR & strUser & "|action" & PARAM_SEPARATOR & strRelease)
        mod_Initialize_AppVars.Item("WritePermission") = TestODBCConnection("tsys_Logins", , strSQL, False)
        ' Notify the user if their back-end privileges are insufficient to use the application
        If mod_Initialize_AppVars.Item("WritePermission") = False And mod_Initialize_AppVars.Item("UserAccessLevel") <> "read only" Then
        ...
        End If
    Else
        mod_Initialize_AppVars.Item("WritePermission") = True
'        strSQL = "INSERT INTO tsys_Logins ( UserName, ActionTaken ) SELECT '" _
'            & strUser & "' AS User, """ & strRelease & """ AS Action;"
'        strSQL = Getmod_Initialize_Applate("i_tsys_logins", "username" & PARAM_SEPARATOR & strUser & "|action" & PARAM_SEPARATOR & strRelease)
        Dim Params(0 To 4) As Variant
        Params(0) = "i_login"
        Params(1) = strUser
        Params(2) = "Application login"
        Params(3) = strReleaseVersion
        Params(4) = mod_Initialize_AppVars.Item("UserAccessLevel")
        
'        strSQL = Getmod_Initialize_Applate("i_login") 'Getmod_Initialize_Applate("i_login", params)
...
'**********************************************
    ...
    If DCount("*", "tsys_App_Releases", "[ID]=" & strReleaseID) = 0 Then
        If mod_Initialize_AppVars.Item("WritePermission") Then BEUpdates (True)
    ...
    ElseIf DCount("*", "tsys_BE_Updates", "[IsDone]=0") > 0 Then
        If mod_Initialize_AppVars.Item("WritePermission") Then BEUpdates (False)
    End If
...
Update_Settings:
    ...
    If DB_ADMIN_CONTROL Then
        ...
        If mod_Initialize_AppVars.Item("HasAccessBE") Then DoCmd.OpenForm "frm_Lock_BE", , , , , acHidden
    
        ' If there is an Access back-end, make the backups button visible
        frm!fsub_DbAdmin.Form!btnBackup.visible = mod_Initialize_AppVars.Item("HasAccessBE")
...
Missing_Table:
    ...
    Select Case missingTable
        ...
        Case "App_Releases", "Bug_Reports", "Logins", "User_Roles"
            ...
            mod_Initialize_AppVars.Item("Connected") = False
        Case ""
    End Select
