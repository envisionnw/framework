...
'               BLC, 5/26/2015 - 1.01 - added mod_db_mod_Db3lates subs/functions - qryExists
'               BLC, 5/26/2016 - 1.02 - added VirtualDAORecordset()
'               BLC, 6/6/2016  - 1.03 - added error handling for duplicate mod_Db3lates, renamed global to g_Appmod_Db3lates
'                                       also added SQL sanitization (escape/replace special chars)
'               BLC, 6/9/2016  - 1.04 - added Createmod_Db3Records()
...
'               BLC, 1/9/2017 - 1.08   - added Setmod_Db3Var()
'               BLC, 1/19/2017 - 1.09  - added RetrieveTableColumnData()
'               BLC, 2/1/2017  - 1.10  - added error handling for improper Getmod_Db3lates()
'                                        parameter syntax (param name:param type)
'               BLC, 2/2/2017  - 1.11  - cleared g_Appmod_Db3lates in Getmod_Db3lates()
...
'                                       g_Appmod_Db3lates, Getmod_Db3late(), Getmod_Db3lates(),
'                                       Createmod_Db3Records(), GetParamsFromSQL(),
' --------------------------------------------------------------------
'               BLC, 3/22/2017          added to Upland db
' --------------------------------------------------------------------
'               BLC, 3/23/2017 - 1.13  - revised Getmod_Db3lates() to use "SQL" vs. "T-SQL" syntax
'               BLC, 3/28/2017 - 1.14  - added CloseObject()
'               BLC, 3/30/2017 - 1.15  - added mod_Db3late dependent query function
'                                        HandleDependentQueries(), SetQueryProperty(),
'                                        DeleteRecord() moved from mod_UI
'               BLC, 3/31/2017 - 1.16  - added g_Appmod_Db3lateIDs global for mod_Db3late/ID matches
...
'                                       revised Getmod_Db3lates to avoid error on dictmod_Db3lates.Add
...
'				Invasives db
'               		BLC, 4/28/2017 - 1.19 - added Delete_All_Records() moved from mod_mod_Db3,
'                       		                reorganized sections, moved SQL_encode(), GetParamsFromSQL()
'                               		        to mod_SQL
'                                       		moved Setmod_Db3Var(), Getmod_Db3VarIndex(),
'		                                        Createmod_Db3Table(), Removemod_Db3Table(),
'        		                                Createmod_Db3Recordset(), Createmod_Db3Records()
'                  		                        to mod_mod_Db3
'               		BLC, 7/18/2017 - 1.20 - Add Refreshmod_Db3Table for updating usys_mod_Db3_transect,
'                                       usys_mod_Db3_speciescover & other mod_Db3 tables w/ std naming
'				Uplands db
...
' Declarations
' ---------------------------------
'   Appmod_Db3lates global dictionary --> defined in std mod_Db3late [mod_Db]
Public g_Appmod_Db3lates As Scripting.Dictionary
Public g_Appmod_Db3lateIDs As Scripting.Dictionary
Public Const PARAM_SEPARATOR As String = ">>"
Public g_OpenQueries As String                  'queries generated by mod_Db3lates (close @ end)
...
'   Tony Toews, unknown
'   http://www.granite.ab.ca/access/mod_Db3tables.htm
'   David Fenton's functino originally based on Tony Toew's function in mod_Db3Tables.MDB
...
On Error GoTo Err_Handler
    ...
    Set rs = CurrentDb.OpenRecordset("usys_mod_Db3_rs", dbOpenDynaset)
...
' ---------------------------------
' FUNCTION:     Getmod_Db3VarIndex
' Description:  Retrieves the index of a mod_Db3Var item
...
'               http://www.accessforums.net/modules/demo-module-vba-code-syntax-using-mod_Db3vars-36353.html
' Adapted:      Bonnie Campbell, Sep 1, 2014
' Revisions:    BLC, 9/1/2014 - initial version
'               BLC, 4/30/2015 - moved from mod_Utilities to mod_Db
' ---------------------------------
Public Function Getmod_Db3VarIndex(stritem) As String
On Error GoTo Err_Handler

Dim i As Integer

    For i = 0 To [mod_Db3Vars].Count - 1
        If [mod_Db3Vars].Item(i).Name = stritem Then
            'fetch the index and exit
            Getmod_Db3VarIndex = i
            Exit Function
        End If
    Next i
    
    'none found -> return -1
    Getmod_Db3VarIndex = -1
...
Err_Handler:
    Select Case Err.Number
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - Getmod_Db3VarIndex[mod_Db])"
    End Select
    Resume Exit_Handler
End Function
...
' ---------------------------------
'  mod_Db3late Related Methods/Functions
' ---------------------------------

' ---------------------------------
' SUB:     Getmod_Db3lates
' Description:  loads mod_Db3lates into memory as a global dictionary object (dictmod_Db3lates)
'               makes current mod_Db3lates available without querying the db tsys_SQL_mod_Db3lates table
' Parameters:   strSyntax - specifies syntax of the mod_Db3late to retrieve (T-SQL, JET, etc.)
'               strParams - specifies the parameters & their datatypes for the mod_Db3late
' Returns:      -
' Assumptions:  -
' Throws:       none
' References:   tsys_Db_mod_Db3lates, Microsoft Scripting Runtime (dictionary object)
'   HansUp, June 27, 2013
'   http://stackoverflow.com/questions/17328092/how-to-display-access-query-results-without-having-to-create-mod_Db3orary-query
' Source/date:  Bonnie Campbell, June 2014
' Revisions:    BLC, 6/16/2014 - initial version
'               BLC, 5/13/2016 - shifted from mod_Db_mod_Db3lates to mod_Db & adjusted to match tsys_Db_mod_Db3lates
'               BLC, 5/19/2016 - revised documentation & renamed Getmod_Db3lates() vs. GetSQLmod_Db3lates() since tsys_Db_mod_Db3lates
...
'               BLC, 6/6/2016  - added error handling for duplicate mod_Db3lates, renamed global to g_Appmod_Db3lates
'               BLC, 2/1/2017  - added error handling for improper parameter syntax (param name:param type)
'               BLC, 2/2/2017  - added clearing of global g_Appmod_Db3lates to allow re-definition
...
'                                to capture these properties of a mod_Db3late
'               BLC, 4/18/2017 - revised Dim to set dictmod_Db3lates as Scripting.Dictionary vs. Dictionary (latter
'                                produces compile error on .Add - Method or Data Member not found)
' ---------------------------------
Public Sub Getmod_Db3lates(Optional strSyntax As String = "", Optional Params As String = "")
    ...
    'sql -> ID, Version, IsSupported, Context, Syntax, mod_Db3lateName, Params, mod_Db3late, Remarks,
    '       EffectiveDate, RetireDate, CreateDate, CreatedBy_ID, LastModified, LastModifiedBy_ID
    strSQL = "SELECT * FROM tsys_Db_mod_Db3lates" & strSQLWhere & ";"
    ...
    'handle no records
    If rs.EOF Then
        MsgBox "Sorry, no mod_Db3lates were found for this database version.", vbExclamation, _
            "Linked Database mod_Db3lates Not Found"
        DoCmd.CancelEvent
        GoTo Exit_Handler
    End If
    ...
    ary(1) = "Context"
    ary(2) = "mod_Db3lateName"
    ary(3) = "mod_Db3late" 'mod_Db3late
    ary(4) = "Params"
    ary(5) = "Syntax"
    ary(6) = "ID"
    ary(7) = "FieldCheck"
    ...
    Dim dictmod_Db3lates As Scripting.Dictionary
    Set dictmod_Db3lates = New Scripting.Dictionary
...
'Debug.Print rs.Fields(ary(i))
                ...
                For j = 0 To UBound(ary2)
                    ...
                    If Not dictParam.Exists(param(0)) And Len(param(0)) <> 0 Then
                        ...
                        If UBound(param) <> 1 Then
                            ...
                            & "|Caption" & PARAM_SEPARATOR & "Invalid SQL mod_Db3late Parameters for the '" & dict("mod_Db3lateName") & "' mod_Db3late"
                            
                            'exit database since application won't function w/o valid mod_Db3lates
                            DoCmd.CloseDatabase
                        End If
        ...
        'add mod_Db3late dictionary to dictionary of mod_Db3lates
        dictmod_Db3lates.Add dict("mod_Db3lateName"), dict
        
'        Debug.Print dict("mod_Db3lateName") & " " & dict.Item("ID")
        rs.MoveNext
    Loop
    
    'load global Appmod_Db3lates As Scripting.Dictionary of mod_Db3lates
    Set g_Appmod_Db3lates = Nothing    'clear first
    
    Set g_Appmod_Db3lates = dictmod_Db3lates
    
Exit_Handler:
    'cleanup
    Set rs = Nothing
    Set dict = Nothing
    Set dictmod_Db3lates = Nothing
    Exit Sub

Err_Handler:
    Select Case Err.Number
      Case 457  'Duplicate mod_Db3late -- tsys_Db_mod_Db3lates finds more than one w/ same name
        MsgBox "A duplicate mod_Db3late was found." & vbCrLf & vbCrLf & _
            "When you click 'OK' a query will run to identify the problem mod_Db3late." & vbCrLf & vbCrLf & _
            ...
            "Error #" & Err.Number & " - Getmod_Db3lates[mod_Db]:" & vbCrLf & _
            Err.Description, vbExclamation, "Duplicate Db mod_Db3late Found! [tsys_Db_mod_Db3lates]"

            Dim strErrorSQL As String
            strErrorSQL = "SELECT mod_Db3lateName, Count(mod_Db3lateName) AS NumberOfDupes " & _
                    "FROM tsys_Db_mod_Db3lates " & _
                    "GROUP By mod_Db3lateName " & _
                    "HAVING Count(mod_Db3lateName) > 1;"

            Dim qdf As DAO.QueryDef
            
            If Not QueryExists("Usysmod_Db3Query") Then
                Set qdf = CurrentDb.CreateQueryDef("Usysmod_Db3Query")
            Else
                Set qdf = CurrentDb.QueryDefs("Usysmod_Db3Query")
            End If
            
            qdf.SQL = strErrorSQL
            
            DoCmd.OpenQuery "USysmod_Db3Query", acViewNormal
      ...
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - Getmod_Db3lates[mod_Db])"
    End Select
    Resume Exit_Handler
End Sub

' ---------------------------------
' FUNCTION:     Getmod_Db3lateIDs
' Description:  retrieves mod_Db3late numeric IDs from mod_Db3lates global mod_Db3late dictionary (Appmod_Db3lates)
'               returns them as a dictionary object with ID:mod_Db3late name as key:value pair
' Parameters:   -
' Returns:      mod_Db3late IDs - mod_Db3late IDs (dictionary object, key=ID, value=mod_Db3late name)
' Assumptions:  tsys_Db_mod_Db3lates contains all desired mod_Db3lates
'               g_Appmod_Db3lates contains mod_Db3late info searchable by mod_Db3lateName which is its key
'               each of the values in that pair is itself a dictionary object containing
'               the various mod_Db3late properties (ID, SQL, Version, etc.)
' Throws:       none
' References:   tsys_Db_mod_Db3lates, Microsoft Scripting Runtime (dictionary object)
...
'               BLC, 3/31/2017 - fix issue which caused g_Appmod_Db3lateIDs to report wrong ID for a mod_Db3late
' ---------------------------------
Public Function Getmod_Db3lateIDs() As Scripting.Dictionary
On Error GoTo Err_Handler

    'initialize Appmod_Db3lates if not populated
    If g_Appmod_Db3lates Is Nothing Then Getmod_Db3lates
    
    Dim d As Scripting.Dictionary, tIDs As Scripting.Dictionary
    Dim i As Integer
    Dim x As Variant
    
    Set d = g_Appmod_Db3lates
    
    Set tIDs = CreateObject("Scripting.Dictionary")
    
    'iterate through the global mod_Db3late dictionary
    'For i = 0 To d.Count - 1
    For Each x In d
    
        'add @ mod_Db3late to the dictionary
        '--------------------------------------------------------------
        ' Note: @ of the global mod_Db3late dictionary's items is itself a dictionary
        '       so reference them via d.Items()(i).Item("keyname")
        '--------------------------------------------------------------
        tIDs.Add d.Item(x).Item("ID"), d.Item(x).Item("mod_Db3lateName")
        'tIDs.Add d.Items()(x).Item("ID"), d.Items()(x).Item("mod_Db3lateName")
        'tIDs.Add d.Items()(i).Item("ID"), d.Items()(i).Item("mod_Db3lateName")
        'tIDs.Add d.Items()(i).Item("ID"), d.Keys()(i)
        ' Debug.Print tIDs.Keys()(i) & " - " & tIDs.Items()(i)
    Next 'i
    
    'set global
    Set g_Appmod_Db3lateIDs = tIDs
    
    Set Getmod_Db3lateIDs = tIDs
...
Err_Handler:
    Select Case Err.Number
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - Getmod_Db3lateIDs[mod_Db])"
    End Select
    Resume Exit_Handler
End Function

' ---------------------------------
' FUNCTION:     Getmod_Db3late
' Description:  retrieves mod_Db3late from mod_Db3lates global mod_Db3late dictionary (Appmod_Db3lates)
' Parameters:   strmod_Db3late - name of mod_Db3late to fetch (string)
'               params - pipe (|) separated parameter listing w/ parameter name:value pairs (: separated) (string)
' Returns:      mod_Db3late - value of the mod_Db3late (string)
'               most mod_Db3lates are SQL strings, so the SQL string (mod_Db3late) field of the given
'               mod_Db3late name is retrieved
' Assumptions:  tsys_Db_mod_Db3lates correctly list parameter:parameter type values & Appmod_Db3lates contain them
...
'               BLC, 6/6/2016  - added error handling for duplicate mod_Db3lates, renamed global to g_Appmod_Db3lates
' ---------------------------------
Public Function Getmod_Db3late(strmod_Db3late As String, Optional Params As String = "") As String
On Error GoTo Err_Handler

    Dim aryParams() As Variant
    Dim ary() As String, ary2() As String
    Dim i As Integer
    Dim mod_Db3late As String, swap As String, param As String

Debug.Print strmod_Db3late

    'initialize Appmod_Db3lates if not populated
    If g_Appmod_Db3lates Is Nothing Then Getmod_Db3lates

    mod_Db3late = g_Appmod_Db3lates(strmod_Db3late).Item("mod_Db3late")
    ...
    If Len(Params) > 0 Then
        ...
        'prepare array of mod_Db3late parameters w/ their data type
        'aryParams = Split(Appmod_Db3lates(strmod_Db3late).item("Params"), "|")
        'Appmod_Db3lates("s_tagline").Item("Params").Item("SourceID") --> integer
        ...
        For i = 0 To UBound(ary)
            ...
            If IsTypeMatch(ary2(1), g_Appmod_Db3lates(strmod_Db3late).Item("Params").Item(ary2(0))) Then
...
'Debug.Print param
                'swap out the placeholder in the mod_Db3late
                mod_Db3late = Replace(mod_Db3late, swap, ary2(1))
...
'Debug.Print mod_Db3late
    
    Getmod_Db3late = mod_Db3late
    ...
    Select Case Err.Number
      Case 457  'Duplicate mod_Db3late -- tsys_Db_mod_Db3lates finds more than one w/ same name
        MsgBox "A duplicate mod_Db3late was found." & vbCrLf & vbCrLf & _
            ...
            "You can close the query after it runs (save it if you like)." & vbCrLf & vbCrLf & _
            "Please contact your data manager to resolve this issue." & vbCrLf & vbCrLf & _
            "Error #" & Err.Number & " - Getmod_Db3late[mod_Db]:" & vbCrLf & _
            Err.Description, vbExclamation, "Duplicate Db mod_Db3late Found! [tsys_Db_mod_Db3lates]"
      ...
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - Getmod_Db3late[mod_Db])"
    End Select
    Resume Exit_Handler
End Function
...
' Parameters:   strmod_Db3late - name of virtual table (string)
...
' ---------------------------------
Public Function VirtualDAORecordset(iCount As Integer, Optional strTable As String = "mod_Db3") As Recordset
On Error GoTo Err_Handler
...
' ---------------------------------
' FUNCTION:     Createmod_Db3Recordset
' Description:  creates a mod_Db3orary DAO recordset
' Parameters:   strmod_Db3late - name of virtual table (string)
'               iCount - number of records (integer)
' Returns:      rs - recordset containing # of records = iCount (DAO.recordset)
' Assumptions:  the mod_Db3orary recordset is used in limited instances when
'               a recordset is needed but doesn't exist
' Throws:       none
' References:
' Source/date:  Bonnie Campbell, June 2016
' Revisions:    BLC, 6/8/2016 - initial version
' ---------------------------------
Public Function Createmod_Db3Recordset(iCount As Integer) As DAO.Recordset
On Error GoTo Err_Handler
...
'    strSQL = "SELECT * FROM usys_mod_Db3_Table;"
    
    Set rs = CurrentDb.OpenRecordset("usys_mod_Db3_Table") 'strSQL, dbOpenSnapshot)
...
Err_Handler:
    Select Case Err.Number
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - Createmod_Db3Recordset[mod_Db])"
    End Select
    Resume Exit_Handler
End Function

' ---------------------------------
' SUB:          Createmod_Db3Records
' Description:  fills a mod_Db3orary table of numbers
'               first clears usys_mod_Db3_table of values, then populates w/ desired set of #s
...
' Revisions:    BLC, 6/8/2016 - initial version
' ---------------------------------
Public Sub Createmod_Db3Records(iStart As Integer, iCount As Integer)
On Error GoTo Err_Handler
    ...
    'clear table
    strSQLDelete = Getmod_Db3late("d_usys_mod_Db3_table")
    ...
    'prep for inserts
    strSQL = Getmod_Db3late("i_usys_mod_Db3_table")
...
Err_Handler:
    Select Case Err.Number
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - Createmod_Db3Records[mod_Db])"
    End Select
    Resume Exit_Handler
End Sub

' ---------------------------------
' SUB:          Createmod_Db3Table
' Description:  creates a mod_Db3 table from an array containing table field definitions
...
' ---------------------------------
Public Sub Createmod_Db3Table(tblName As String, aryFields() As Variant)
On Error GoTo Err_Handler
    ...
    If TableExists(tblName) Then Removemod_Db3Table (tblName)
...
Err_Handler:
    Select Case Err.Number
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - Createmod_Db3Table[mod_Db])"
    End Select
    Resume Exit_Handler
End Sub

' ---------------------------------
' SUB:          Removemod_Db3Table
' Description:  removes a mod_Db3 table from database
' Assumptions:  -
' Parameters:   tblName - table name (string)
' Returns:      -
' References:   -
...
' Revisions:    BLC, 9/20/2016 - initial version
' ---------------------------------
Public Sub Removemod_Db3Table(tblName As String)
On Error GoTo Err_Handler
...
Err_Handler:
    Select Case Err.Number
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - Removemod_Db3Table[mod_Db])"
    End Select
    Resume Exit_Handler
End Sub
...
On Error GoTo Err_Handler
    ...
    strSQL = Getmod_Db3late("d_form_record", "tbl" & PARAM_SEPARATOR & tbl & "|id" & PARAM_SEPARATOR & ID)
...
' ---------------------------------
' SUB:          Setmod_Db3Var
' Description:  Checks if mod_Db3Var exists, creates it if not, & sets value
' Assumptions:  -
' Parameters:   strVar - mod_Db3Var name (string)
...
' ---------------------------------
Public Sub Setmod_Db3Var(strVar As String, val As Variant)
On Error GoTo Err_Handler

    If Not mod_Db3Vars(strVar) Is Nothing Then
        mod_Db3Vars(strVar) = val
    Else
        mod_Db3Vars.Add strVar, val
    End If
...
Err_Handler:
    Select Case Err.Number
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - Setmod_Db3Var[mod_Db])"
    End Select
    Resume Exit_Handler
End Sub
...
' Description:  Runs or closes mod_Db3late dependent queries
...
' Revisions:
'   BLC - 3/30/2017 - initial version
'   BLC - 3/31/2017 - adjust to include check for g_Appmod_Db3lateIDs
...
End If
    ...
    If IsArray(ary) Then
        ...
        Dim qdf As DAO.QueryDef
        Dim rs As DAO.Recordset
        Dim strmod_Db3late As String, deps2 As String, strSQL As String
        Dim tID As Integer, imod_Db3late
        
        Set db = CurrentDb
        
        'retrieve mod_Db3late ID dictionary
        'initialize Appmod_Db3lates if not populated
        If g_Appmod_Db3lateIDs Is Nothing Then Getmod_Db3lateIDs
        Set ids = g_Appmod_Db3lateIDs
        ...
        For i = LBound(ary) To UBound(ary)
            ...
            imod_Db3late = ids(tID)
            
            strmod_Db3late = g_Appmod_Db3lates(imod_Db3late).Item("mod_Db3lateName")
            
            Select Case LCase(action)
                Case "run"  'generates dep queries
                    
                    'handle dependencies of dependencies first
                    deps2 = g_Appmod_Db3lates(strmod_Db3late).Item("Dependencies")

                    If Len(deps2) > 0 Then HandleDependentQueries deps2, "run"
                
                    'retrieve mod_Db3late SQL
                    strSQL = g_Appmod_Db3lates(strmod_Db3late).Item("mod_Db3late")
                
                    'create & run query
                    'qdf.Name = strmod_Db3late 'rs("mod_Db3lateName")
                    'qdf.SQL = strSQL 'rs("mod_Db3late")
                    
                    'check if query exists
                    If Not QueryExists(strmod_Db3late) Then 'db.QueryDefs(strmod_Db3late) Is Nothing Then 'qdf.Name) Is Nothing Then
                        'create query
                        db.CreateQueryDef strmod_Db3late, strSQL 'qdf.Name, qdf.SQL
                        
                        'hide new query
'                        db.QueryDefs (strmod_Db3late)
                        Application.SetHiddenAttribute acQuery, strmod_Db3late, True
                ...
                Case "remove"   'remove dep queries
                    ...
                    If QueryExists(strmod_Db3late) Then
                        'close & remove
                        DoCmd.Close acQuery, strmod_Db3late, acSaveNo 'qdf.Name, acSaveNo
                        DoCmd.DeleteObject acQuery, strmod_Db3late 'qdf.Name
...
' ---------------------------------
' SUB:          Removemod_Db3lateQueries
' Description:  Removes queries created from mod_Db3lates
' Assumptions:  -
' Parameters:   -
' Returns:      -
' Throws:       none
...
'   BLC - 3/30/2017 - initial version
' ---------------------------------
Public Sub Removemod_Db3lateQueries()
On Error GoTo Err_Handler
    
    Dim i As Integer
    
    'initialize Appmod_Db3lates if not populated (necessary for testing)
    If g_Appmod_Db3lates Is Nothing Then Getmod_Db3lates
    
    For i = 0 To g_Appmod_Db3lates.Count - 1
    
        Dim strmod_Db3late As String
        Dim imod_Db3late As Integer
         
        strmod_Db3late = g_Appmod_Db3lates.Items()(i).Item("mod_Db3lateName")
        imod_Db3late = g_Appmod_Db3lates.Items()(i).Item("ID")
        ...
        If QueryExists(strmod_Db3late) Then
            ...
            'remove from open query list
            g_OpenQueries = Replace(Replace(g_OpenQueries, imod_Db3late, ""), ",,", ",")
        End If
...
Err_Handler:
    Select Case Err.Number
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - Removemod_Db3lateQueries[mod_Db])"
    End Select
    Resume Exit_Handler
End Sub
...
' ---------------------------------
' Sub:          Refreshmod_Db3Table
' Description:  Refreshes mod_Db3 table data
' Assumptions:  mod_Db3 table is generated by a query
'               w/ name Create_* where * = mod_Db3 table name
' Parameters:   tbl - mod_Db3 table name (string)
...
' ---------------------------------
Public Sub Refreshmod_Db3Table(tbl As String, _
...
On Error GoTo Err_Handler
    ...
    're-generate the mod_Db3 table source
    DoCmd.SetWarnings False
...
Err_Handler:
    Select Case Err.Number
      Case Else
        MsgBox "Error #" & Err.Number & ": " & Err.Description, vbCritical, _
            "Error encountered (#" & Err.Number & " - Refreshmod_Db3Table[mod_Db])"
    End Select
    Resume Exit_Handler
End Sub
