...
'               BLC, 6/12/2015 - 1.04 - replaced mod_UserVars.item(... with mod_UserVars("...
...
'               BLC, 6/30/2016 - 1.08 - adjusted to use Getmod_Userlate
'               BLC, 9/26/2016 - 1.09 - adjusted to use GetRecords() vs. Getmod_Userlate()
'               BLC, 10/5/2016 - 1.10 - revised to set UserAccessLevelID mod_UserVar
' ------------------------------------------------------------------------------
...
' Description:  Retrieves user access level from tsysUser & sets mod_UserVars.Item("UserAccessLevel")
'               If User_role exists, this value is set otherwise
'               mod_UserVars.Item("UserAccessLevel") defaults to "read only"
' Parameters:   -
...
'               BLC, 6/12/2015 - replaced mod_UserVars.item("... with mod_UserVars("...
'               BLC, 4/4/2016  - changed Exit_Function > Exit_Handler, dbCurrent to CurrentDb
'               BLC, 6/30/2016 - revised to use Getmod_Userlate()
'               BLC, 10/5/2016 - revised to set UserAccessLevelID mod_UserVar
' ------------------------------------------------------------------------------
'				BLC, 8/23/2017 - 1.11 - merge prior work:
...
'        Set rs = CurrentDb.OpenRecordset(strSQL)
        ...
        With db
            Set qdf = .QueryDefs("usys_mod_User_qdf")
                    
            With qdf
                .SQL = Getmod_Userlate("s_user_access")
        ...
        If Not rs.BOF And Not rs.EOF Then
            'db user role
            mod_UserVars("UserAccessLevel") = CStr(rs!AccessLevel)
            mod_UserVars("UserAccessLevelID") = CLng(rs!AccessLevelID)
        Else
            'default
            mod_UserVars("UserAccessLevel") = "read only"
            mod_UserVars("UserAccessLevelID") = 4 'ID for read only (see Access table)
        End If
...
'        If Not rs.BOF And Not rs.EOF Then
'            'db user role
'            mod_UserVars("UserAccessLevel") = CStr(rs!User_role)
'        Else
'            'default
'            mod_UserVars("UserAccessLevel") = "read only"
'        End If
    ...
    Else
        ...
        mod_UserVars("UserAccessLevel") = "admin"
    End If
    
    getDbUserAccess = mod_UserVars("UserAccessLevel")
...
' ---------------------------------
' SUB:          setUserAccess
' Description:  Sets form control access/functionality based on global user access level - mod_UserVars.Item("UserAccessLevel")
...
On Error GoTo Err_Handler
    ...
    ' set defaults
    '-------------------------------
Debug.Print "useraccesslvl=" & mod_UserVars("UserAccessLevel")
...
'-------------------------------
    ...
    '-------------------------------
        Select Case mod_UserVars("UserAccessLevel")
                    ...
                    Case "frm_Switchboard"
                        ...
                        If flag = "update" Then
                            With !fsub_DbAdmin.Form
                                ...
                                !tbxAppMode = mod_UserVars("UserAccessLevel")
...
'               BLC, 6/12/2015 - replaced mod_UserVars.item("... with mod_UserVars("...
'               BLC, 6/30/2016 - revised to use SWITCHBOARD vs. frm_Switchboard and Getmod_Userlate()
' ---------------------------------
    ...
    With frm
        ...
        Select Case .Name
            Case SWITCHBOARD
                ...
                If mod_UserVars("Connected") And mod_UserVars("WritePermission") Then
'                    strSQL = "INSERT INTO tsys_Logins ( User_name, Action_taken ) SELECT '" _
'                        & Environ("Username") & "' AS User, 'close' AS Action;"
'                    strSQL = Getmod_Userlate("tsys_login_action")
                    Dim db As DAO.Database
                    Dim qdf As DAO.QueryDef
                    ...
                    With db
                        ...
                        With qdf
                            .SQL = Getmod_Userlate("tsys_login_action")
...
'               BLC, 9/26/2016 - adjusted to use GetRecords vs. Getmod_Userlate
' =================================
Public Function AccessID(AccessLevel As String) As Long
    ...
    'add mod_User mod_Uservar!
    mod_UserVars.Add "mod_UserLvl", LCase(AccessLevel)
    'strSQL = Getmod_Userlate("s_access_level", "lvl" & PARAM_SEPARATOR & LCase(AccessLevel))
